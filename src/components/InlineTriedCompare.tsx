    ...
    <section className="mt-4 rounded-lg border border-border bg-card p-4">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <InlineWinnerButton
          ...
          onPick={() => mut.mutate(dish.id)}
        />
        <InlineWinnerButton
          dish={other}
          disabled={mut.isPending}
          onPick={() => mut.mutate(other.id)}
        />
      </div>
    </section>
  );
    ...
    >
      <div className="aspect-[4/3] bg-muted">
        {dish.photo_url ? (
          ...
          <div className="flex h-full items-center justify-center bg-secondary type-card-title text-muted-foreground">
            JaanNee
          </div>
        )}
      </div>
