...
DO $$
DECLARE
  slugs text[] := ARRAY['sushi','dim-sum','tapas','korean-bbq','steak','specialty-coffee','matcha'];
  s text; found int;
BEGIN
  FOREACH s IN ARRAY slugs LOOP
    ...
    IF found = 0 THEN RAISE NOTICE 'category slug % is missing — not creating a replacement', s; END IF;
  END LOOP;
END $$;

-- 2. dish_ranking_key + subtype-scoped helper
...
BEGIN
  ...
  IF d.category_id IS NULL THEN RETURN NULL; END IF;
...
-- 3. Tried-before-voting on ALL updates
...
DECLARE
  ...
  moved_reports int := 0;
BEGIN
  IF _keep_id IS NULL OR _remove_id IS NULL THEN RAISE EXCEPTION 'Both dish ids are required'; END IF;
...
-- 6. normalize_dish_name + unique index rebuild
...
BEGIN
  FOR cat_row IN SELECT * FROM jsonb_array_elements(seeds) LOOP
    ...
    FOR sub_row IN SELECT * FROM jsonb_array_elements(cat_row->'subs') LOOP
      ...
      VALUES (cat_id, sub_row->>'slug', sub_row->>'en', sub_row->>'th', true, (sub_row->>'o')::int)
...
-- 8. nearby_places (4-arg, bbox, caps, service-role only)
...
BEGIN
  IF _lat IS NULL OR _lng IS NULL THEN RAISE EXCEPTION 'Latitude and longitude are required'; END IF;
  IF _lat < -90 OR _lat > 90 THEN RAISE EXCEPTION 'Latitude must be between -90 and 90'; END IF;
  IF _lng < -180 OR _lng > 180 THEN RAISE EXCEPTION 'Longitude must be between -180 and 180'; END IF;
  IF _radius_km IS NULL OR _radius_km <= 0 THEN RAISE EXCEPTION 'Radius must be positive'; END IF;
