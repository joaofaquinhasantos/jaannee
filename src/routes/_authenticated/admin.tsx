...
function Taxonomy() {
  const { t } = useI18n();
  const qc = useQueryClient();
  const [c, setC] = useState<{ slug: string; name_en: string; name_th: string; cuisine: string; requires_subtype: boolean; reference_photo_url: string }>({ slug: "", name_en: "", name_th: "", cuisine: "", requires_subtype: false, reference_photo_url: "" });
  ...
  const [editing, setEditing] = useState<
    | { kind: "category" | "area"; slug: string; name_en: string; name_th: string; cuisine?: string; requires_subtype?: boolean; reference_photo_url?: string }
    | null
  >(null);
  ...
  const cMut = useMutation({
    ...
    onSuccess: () => {
      toast.success("Saved");
      setC({ slug: "", name_en: "", name_th: "", cuisine: "", requires_subtype: false, reference_photo_url: "" });
      qc.invalidateQueries({ queryKey: ["admin-categories"] });
      qc.invalidateQueries({ queryKey: ["categories"] });
    },
    onError: (e: any) => toast.error(e.message),
  ...
  const editMut = useMutation({
    mutationFn: async () => {
      ...
      if (editing.kind === "category") payload.reference_photo_url = editing.reference_photo_url ?? "";
      ...
      <div className="rounded-lg border border-border bg-card p-4">
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="rounded-md border border-border bg-background p-3">
            ...
            <p className="mt-1 text-xs text-muted-foreground">A visual example for category navigation. It does not enter rankings.</p>
            {c.reference_photo_url && (
              <img src={c.reference_photo_url} alt="" className="mt-2 h-28 w-full rounded-md object-cover" />
            )}
            <div className="mt-2 flex flex-wrap items-center gap-2">
              <Input
                ...
                onChange={async (e) => {
                  ...
                  if (file) await uploadCategoryPhoto(file, (url) => setC((current) => ({ ...current, reference_photo_url: url })));
                  e.currentTarget.value = "";
                }}
              />
              {c.reference_photo_url && (
                <Button type="button" size="sm" variant="outline" onClick={() => setC({ ...c, reference_photo_url: "" })}>
                  Remove
                </Button>
              )}
            </div>
                        ...
                        <div key={row.slug} className="px-3 py-3 text-sm">
                          ...
                          <div className="flex items-center justify-between gap-3">
                            {row.reference_photo_url && (
                              <img src={row.reference_photo_url} alt="" className="h-12 w-16 shrink-0 rounded-md object-cover" />
                            )}
                            ...
                            <div className="flex shrink-0 gap-2">
                              <Button size="sm" variant="outline" onClick={() => setEditing({ kind: "category", slug: row.slug, name_en: row.name_en, name_th: row.name_th, cuisine: row.cuisine || "", requires_subtype: !!row.requires_subtype, reference_photo_url: row.reference_photo_url || "" })}>Edit</Button>
          ...
          {editing && (
            <div className="space-y-3">
              ...
              {editing.kind === "category" && (
                <div className="rounded-md border border-border bg-background p-3">
                  <Label>Reference photo</Label>
                  <p className="mt-1 text-xs text-muted-foreground">Used only as a category example, never as a ranked dish.</p>
                  {editing.reference_photo_url && (
                    <img src={editing.reference_photo_url} alt="" className="mt-2 h-36 w-full rounded-md object-cover" />
                  )}
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    <Input
                      type="file"
