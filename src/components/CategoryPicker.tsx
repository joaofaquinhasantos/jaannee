...
export function CategoryPicker({
  ...
  const secondaryLabel = (category: any) =>
    lang === "th"
      ? category.name_en !== categoryLabel(category)
        ? category.name_en
        : null
      : category.name_th;
  ...
  return (
    <>
      ...
      >
        ...
        >
          {triggerLabel || (selected ? categoryLabel(selected) : placeholder)}
        </span>
        <Search
            ...
            {groups.map(([cuisine, items]) => (
              <div key={cuisine} className="py-2">
                ...
                <div className="mt-1 space-y-1">
                  {items.map((category) => {
                    const secondary = secondaryLabel(category);
                    return (
                      <button
                        key={category.id}
                        type="button"
